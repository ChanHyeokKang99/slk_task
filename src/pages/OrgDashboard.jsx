  ...
  return (
    <div style={s.page}>
      <div style={s.header}>
        ...
        <div style={{ ...s.providerBadge, background: currentOrg.apiConfigured ? "#F0FDF4" : "#FEF2F2", border: `1px solid ${currentOrg.apiConfigured ? "#86EFAC" : "#FECACA"}`, color: currentOrg.apiConfigured ? "#16A34A" : "#DC2626" }}>
          {currentOrg.apiConfigured ? `✓ 연동 중 · ${currentOrg.messageProvider === "cloudmessage" ? "클라우드메시지" : "센드톡"}` : "⚠ 업체 미설정"}
        </div>
      </div>
...
const s = {
  ...
  providerBadge: { fontSize: 13, fontWeight: 600, padding: "6px 14px", borderRadius: 20 },
  ...
  th: { textAlign: "left", padding: "8px 12px", fontSize: 11, color: "#94A3B8", fontWeight: 600, borderBottom: "1px solid #F1F5F9", textTransform: "uppercase" },
